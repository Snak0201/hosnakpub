...
class BureauDetailViewTest(TestCase):
    ...
    def test_does_not_have_link_to_draft_articles(self):
        ...
        response = self.client.get(reverse("articles:bureau", kwargs={"slug": self.bureau.slug}))
        self.assertEqual(self.response.context["articles"].count(), response.context["articles"].count())
